...
export function Card({ user, onfollow, isFollowed }) {
  ...
  return (
    <ListItem>
      <TwitsCount>{`${user.twits} Tweets`}</TwitsCount>
      <FollowersCount>{`${transformNumber(user.followers)} Followers`}</FollowersCount>
      <BtnContainer>
        <Button onClick={() => onfollow(updatedUser)} style={isFollowed ? { backgroundColor: '#5CD3A8' } : null}>
          <BtnText>{isFollowed ? `FOLLOWING` : `FOLLOW`}</BtnText>
        </Button>
      </BtnContainer>
      <Line />
